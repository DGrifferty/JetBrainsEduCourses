...
conn = sqlite3.connect('Banking_Database.s3db')
cur = conn.cursor
#
# conn.execute('''CREATE TABLE BANK_DATABASE
# (ID INT PRIMARY KEY NOT NULL,
# CARD_NUM INT NOT NULL,
# PIN INT NOT NULL,
# BALANCE INT NOT NULL);''')

data = cur.execute("SELECT id, card_num, PIN, BALANCE from BANK_DATABASE")
data.fetchall()
...
while True:
    ...
    if logged_in == False:
        ...
        if user_choice == 1:
            ...
            conn.execute(f"INSERT INTO BANK_DATABASE(ID, CARD_NUM, PIN, BALANCE) \
                         VALUES ({len(id_lst)}, {card_num}, {pin}, 0)")
